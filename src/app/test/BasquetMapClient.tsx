...
export default function BasquetMapClient() {
  const [points, setPoints] = useState<BasquetPoint[]>([]);
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = "basquet.geojson";
    link.click();
    URL.revokeObjectURL(url);
  };

  const geocode = useCallback(async (raw: BasquetPoint[]) => {
    if (!raw.length) {
      return [] as BasquetPoint[];
    }
    const url = "/api/basquet-geocode";
    ...
    return (await geocodedResponse.json()) as BasquetPoint[];
  }, []);
  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      const raw = (await uploadResponse.json()) as BasquetPoint[];
      if (!raw.length) {
        throw new Error("No se encontraron filas en la hoja Basquet");
      }
      return raw;
    },
    [],
  ...
  const geocodeInBatches = useCallback(
    async (raw: BasquetPoint[], batchSize = 5) => {
      const total = raw.length;
      setProcessedCount(0);
      const collected: BasquetPoint[] = [];
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {!loading && rawCount === 0 ? (
        <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-6 text-sm text-[color:var(--text-2)]">
          Subi un Excel con la hoja Basquet para iniciar.
        </div>
      ) : null}
